...
class AppBase:
    ...
    def set_user_credentials(self, user_credentials):
    ...
    def create_widgets(self):
        ...
        self.combobox1['values'] = list(self.get_prices().keys())
    ...
    def calculate_billing(self):
        ...
        if not nama or not password:
            ...
            return
        if nama in self.get_user_credentials() and self.get_user_credentials()[nama] != password:
            messagebox.showwarning("Peringatan", "Password salah untuk nama pengguna ini")
            return
        
        self._user_credentials[nama] = password  

        if paket in self.get_prices():
            harga = self.get_prices()[paket]
            self._history.append((nama, paket))
...
class BillingApp(AppBase):
    ...
    def show_history(self):
        if self.get_history():
            history_message = "\n".join([f"{nama} - {paket}" for nama, paket in self.get_history()])
    ...
    def clear_history(self):
        self.set_history([])
        messagebox.showinfo("History", "Antrian telah dikosongkan.")

    def create_widgets(self):
        super().create_widgets()
        self.history_button = Button(self.root, text="Show History", command=self.show_history, bg='lightblue')
        self.history_button.pack(pady=10)
        self.clear_history_button = Button(self.root, text="Clear History", command=self.clear_history, bg='lightblue')
